...
const About = () => {
  ...
  const socialLinks = [
    { icon: <FaFacebook />, url: 'https://www.facebook.com/share/1DHirebiGM/?mibextid=wwXIfr' },
    { icon: <FaInstagram />, url: 'https://www.instagram.com/ricc.a__/?igsh=MXZrZTBtdXNzN2J1Yg%3D%3D&utm_source=qr' },
    { icon: <FaInstagram />, url: 'https://www.instagram.com/perera__diary/?igsh=ZnR5ZGFrY3QxdWhv&utm_source=qr' },
    { icon: <FaWhatsapp />, url: 'https://wa.me/94765587168' },
  ];
    ...
    >
      <div className="max-w-7xl mx-auto">
        ...
        <motion.div
          ...
          className="mb-8"
        >
          <p className={`${isDark ? 'text-gray-300' : 'text-gray-700'} text-sm md:text-base leading-relaxed font-body text-center max-w-4xl mx-auto`}>
        ...
        >
          <h3 className={`text-lg md:text-xl font-semibold ${isDark ? 'text-white' : 'text-gray-900'} mb-6 font-body`}>
            Beyond Coding
          </h3>
          <div className="space-y-4">
            <div className="flex flex-wrap gap-4 justify-center">
              {['Photography', 'Music', 'Singing'].map((interest, index) => (
                <span 
                  key={index}
                  className={`${isDark ? 'text-gray-300' : 'text-gray-700'} text-sm md:text-base font-body`}
                >
                  {interest}{index < 2 ? ' •' : ''}
                </span>
              ))}
